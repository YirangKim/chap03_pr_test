...
public class ParameterizedTests {
     ...
     *  parameterized-test는 @ParameterizedTest 어노테이션을 @Test 어노테이션을 데체하여 사용하며,
    ...
    @ParameterizedTest
    @ValueSource(ints = {2, 3, -1, 15, 123})
    void testIsOdd(int number) {

        //when
        boolean result = NumberValidator.isOdd(number); //isOdd 홀수 메소드
    ...
    void ParameterizedTest(Integer limt){
    ...
    void testIsNull(String input){
        ...
        //then
        Assertions.assertTrue(result); //Assertions.assertTrue 값이 트루면 통과
    }
    ...
    @EnumSource(
            ...
            mode = EnumSource.Mode.EXCLUDE //EXCLUDE names 일치하는 얘들 빼기, 1월 3월.. 만 테스트 하겠다
            //Enum 타입
    )
    ...
    @ParameterizedTest
    //@CsvSource({"test,TEST", "tEsT, TEST", "Javascript, JAVASCRIPT"}) // 쉼표로 구분
    ...
    void testToUpperCase(String input, String verifyValue){
        ...
        //when
        String actual = input.toUpperCase(); //toUpperCase 대문자로

        //then
        Assertions.assertEquals(verifyValue, actual);
    }

    @DisplayName("CSV파일을 읽은 테스트 데이터를 테스트에 활용하는 지 확인")
    @ParameterizedTest
    ...
    void testToUpperCaseWithCSVFileData(String input, String verifyValue){
        ...
        //when
        String actual = input.toUpperCase(); //toUpperCase 대문자 만드는 메소드
    ...
    /* 필기.
     *   JUnit5는 인수로 지정된 String을 Enum 변환한다. 이처럼 기본적으로 제공하는 변환을 암시적 변환이라고 한다.
     *   UUID, LocalDate, LocalDateTime, Year, Month, 파일 및 경로, URL, 열거형 서브클래스 들을 암시적으로 변환해준다.
     */

    @DisplayName("암시적 변환 메소드")
    @ParameterizedTest(name = "[{0}] is 30 days long")
    @CsvSource({"APRIL", "JUNE", "SEPTEMBER", "NOVEMBER"})
    void testAutoConverting(Month month){ //enum타입으로 바뀜

        //given
        int verifyValue = 30;

        //when
        int actual = DateValidator.getLastDayof(month); //getLastDayof 마지막 날

        //then
        Assertions.assertEquals(verifyValue, actual);  //마지막 날이 30일인지 확인
    }
